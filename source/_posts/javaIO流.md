...
![img](https://image.itbaima.net/markdown/2023/08/14/VHOBIXvulUsMYRT.jpg)
...
![img](https://image.itbaima.net/markdown/2023/08/14/UpicNGRSy7Zz35f.jpg)
...
```java
 public static void main(String[] args) {
        try (BufferedReader reader = new BufferedReader(new FileReader("test.txt"))){//这里的路径指的是target统计的目录
            reader
                    .lines()//读取行
                    .limit(2)//限定前两行而不是前两个字符
                    .distinct()//对行之间去重而不是每行
                    .sorted()
...
public static void main(String[] args) {
        try (BufferedWriter reader = new BufferedWriter(new FileWriter("output.txt"))){
            /**
             * 通过使用newLine()方法，可以保持跨平台的兼容性，使得生成的文本文件在不同操作系统上都可以正确地显示换行符。
             *
             * 所以，在你的代码中调用reader.newLine()方法插入换行符，以便在写入文本时进行换行。这样，写入的文本会按照每行一个字符串的形式，并且每行结束后会自动换行。
             */
            reader.write("汉堡做滴彳亍不彳亍");   //可以直接写入一个字符串
            reader.newLine();   //使用newLine进行换行，

            reader.write("i love you");   //可以直接写入一个字符串
            reader.flush();   //清空缓冲区
        }catch (IOException e) {
            e.printStackTrace();
        }
...
![img](https://image.itbaima.net/markdown/2023/08/14/BNg21b7woepEVkY.png)
...
public static void main(String[] args) {
        try (DataInputStream dataInputStream = new DataInputStream(new FileInputStream("test.txt"))){
            System.out.println(dataInputStream.readChar());   //直接将数据读取为任意基本数据类型,这里读取的是boolean类型的指
//            System.out.println(dataInputStream.readInt());   //直接将数据读取为任意基本数据类型,这里读取的是boolean类型的指
...
static class People implements Serializable{
    private static final long serialVersi onUID = 123456;   //在序列化时，会被自动添加这个属性，它代表当前类的版本，我们也可以手动指定版本。
